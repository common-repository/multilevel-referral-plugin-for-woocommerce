...
<td>{expire_date}</td>
		...
		public function register_hook_callbacks() {
			...
			add_action( 'upgrader_process_complete', array($this, 'wmc_upgrade_completed' ), 10, 2 );
		}
		...
		function wmc_upgrade_completed( $upgrader_object, $options ) {
			/* global $wpdb;
		...
		function wmc_custom_activate_plugin(){
			...
			if( $wmc_version != WMC_VER ){
				...
				$istransfer = $wpdb->get_results(  "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '".$table_name."' AND column_name = 'is_transfer'");

				if(empty($istransfer))
				{  
					$sql = $wpdb->query("ALTER TABLE {$table_name} ADD is_transfer tinyint(1) NOT NULL AFTER redeems"); 
				}

				update_option( 'wmc_version', WMC_VER );
			}
			if( mrapfw_fs()->is_free_plan() ){
				if(strpos( $_SERVER['REQUEST_URI'], 'post-new.php' ) !== false && sanitize_text_field($_GET['post_type']) == 'wmc-banner' ){
					wp_redirect( admin_url( '/edit.php?post_type='.sanitize_text_field($_GET['post_type']) ) );
					exit();
				}
			}
		}
			...
			try {
				if( is_admin() ){
                    ...
                    if(isset($_GET['changeReferralsLevel'])){
                        $obj_referal_users = new Referal_Users();
                        $userId=sanitize_text_field($_GET['changeReferralsLevel']);
                        $obj_referal_users->change_referral_user_level($userId);
            ...
            if(is_array($allBanners) && count($allBanners)>0){
                foreach($allBanners as $name=>$fullpath){
                    ...
                    if(is_array($arrPreBanners) && count($arrPreBanners)>0){
						...
						update_option('wmc-default-banners',array_chunk($arrPreBanners,3)[0],true);
                    }
                }
            }
        }
        ...
        static function remove_banner_row_actions($actions, $post ){
            if( get_post_type() === 'wmc-banner' ){
				...
				if( mrapfw_fs()->is_free_plan() ){
					$arrDefaultBanners=get_option('wmc-default-banners');                
					if(is_array($arrDefaultBanners) && count($arrDefaultBanners)>0){
						if(!in_array($post->ID,$arrDefaultBanners)){
							foreach ($actions as $key => $value) {
								$actions[ $key ] = sprintf( '<a href="#" class="%s">%s</a>', 'premium_featured' , strip_tags( $value ) );
							}
						}
					}
				}
        ...
        static function fnDisplayPostState( $post_states, $post ){
        	$arrDefaultBanners=get_option('wmc-default-banners');
        	if(is_array($arrDefaultBanners) && count($arrDefaultBanners)>0){
                if(!in_array($post->ID,$arrDefaultBanners) && (isset($_GET['post_type']) && sanitize_text_field($_GET['post_type']) == 'wmc-banner') ){
                	printf( '&nbsp;<span>( %s )</span>&nbsp;', __('Premium Banner','multilevel-referral-affiliate') );
                }
            }
        }
